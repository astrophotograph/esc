            ...
            while time.time() < end_time:
                try:
                    ...
                    message = await asyncio.wait_for(
                        self.ws.recv(), timeout=min(remaining_time, 1.0)
                    )
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                for telescope in telescopes:
                    if (
                        telescope.get("id") == telescope_id
                        or telescope.get("serial_number") == telescope_id
                    ):
                ...
                if target_telescope:
                    print(
                        f"✅ Found target telescope: {json.dumps(target_telescope, indent=2)}"
                    )
                else:
                    print(f"❌ Telescope {telescope_id} not found in list")
                    print(
                        f"Available telescopes: {[t.get('id', 'no-id') for t in telescopes]}"
                    )
