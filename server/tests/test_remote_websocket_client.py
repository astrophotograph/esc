...
from remote_websocket_client import (
    RemoteController,
    RemoteWebSocketClient,
    RemoteWebSocketManager,
)
...
class TestRemoteController:
    ...
    def test_remote_controller_defaults(self):
        ...
        controller = RemoteController(
            host="test.host", port=8000, telescope_id="test", controller_id="test"
        )
...
class TestRemoteWebSocketManager:
    ...
    async def test_add_remote_controller(self, manager, controller):
        ...
        with patch.object(
            RemoteWebSocketClient, "connect", return_value=True
        ) as mock_connect:
            with patch.object(
                RemoteWebSocketClient, "send_subscription"
            ) as mock_subscribe:
    ...
    async def test_send_to_telescope_no_connection(self, manager):
        ...
        with pytest.raises(ConnectionError):
            await manager.send_to_telescope(
                "nonexistent_telescope", {"test": "message"}
            )
    ...
    def test_force_reconnect_telescope(self, manager, controller):
        ...
        result = manager.force_reconnect_telescope(
            controller.telescope_id, "Test reason"
        )
