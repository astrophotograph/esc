...
@pytest.mark.skipif(not WEBSOCKET_COMMANDS_AVAILABLE, reason="WebSocket commands not available")
class TestWebSocketCommandExecution:
    ...
    @pytest.mark.asyncio
    async def test_handle_control_command_goto(self, manager, mock_connection, mock_telescope_client):
    ...
    @pytest.mark.asyncio
    async def test_handle_control_command_park(self, manager, mock_connection, mock_telescope_client):
    ...
    @pytest.mark.asyncio
    async def test_handle_control_command_move(self, manager, mock_connection, mock_telescope_client):
    ...
    @pytest.mark.asyncio
    async def test_handle_control_command_focus(self, manager, mock_connection, mock_telescope_client):
    ...
    @pytest.mark.asyncio
    async def test_handle_control_command_telescope_not_found(self, manager, mock_connection):
        ...
        if sent_calls:
            ...
            assert "error" in sent_data.get("type", "").lower() or "error" in str(sent_data)
    
    @pytest.mark.asyncio
    async def test_handle_control_command_telescope_disconnected(self, manager, mock_connection, mock_telescope_client):
    ...
    @pytest.mark.asyncio
    async def test_handle_control_command_error(self, manager, mock_connection, mock_telescope_client):
        """Test handling control command that results in error."""
        telescope_id = "error_telescope"
        manager.register_telescope_client(telescope_id, mock_telescope_client)
        
        # Mock command error
        mock_telescope_client.send_command.side_effect = Exception("Telescope communication error")
    ...
    async def test_execute_move_command(self, manager, mock_telescope_client):
        ...
        if hasattr(manager, '_execute_move_command'):
            result = await manager._execute_move_command(
                telescope_id, {"direction": "north", "duration": 5, "speed": 1}, "move_123"
            )
...
class TestWebSocketSubscriptionHandling:
    ...
    async def test_handle_subscribe(self, manager, mock_connection):
        ...
        if hasattr(manager, '_handle_subscribe'):
            ...
            assert mock_connection.is_subscribed_to("test_telescope", SubscriptionType.STATUS)
            assert mock_connection.is_subscribed_to("test_telescope", SubscriptionType.IMAGING)
    ...
    async def test_handle_subscribe_all_telescopes(self, manager, mock_connection):
        ...
        if hasattr(manager, '_handle_subscribe'):
            ...
            assert mock_connection.is_subscribed_to("telescope1", SubscriptionType.STATUS)
            assert mock_connection.is_subscribed_to("telescope2", SubscriptionType.STATUS)
    ...
    async def test_handle_unsubscribe(self, manager, mock_connection):
        ...
        mock_connection.add_subscription("test_telescope", [SubscriptionType.STATUS, SubscriptionType.IMAGING])
        ...
        if hasattr(manager, '_handle_unsubscribe'):
            ...
            assert not mock_connection.is_subscribed_to("test_telescope", SubscriptionType.STATUS)
            assert mock_connection.is_subscribed_to("test_telescope", SubscriptionType.IMAGING)
    ...
    async def test_heartbeat_loop(self, manager, mock_connection):
        ...
        if hasattr(manager, '_heartbeat_loop'):
            ...
            with patch('asyncio.sleep', new=AsyncMock()) as mock_sleep:
                    ...
                    if manager.connections:
                        for connection in list(manager.connections.values()):
                            if connection.is_alive:
                                await connection.websocket.send_text('{"type": "heartbeat"}')
                ...
                mock_connection.websocket.send_text.assert_called_with('{"type": "heartbeat"}')
