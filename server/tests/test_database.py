...
class TestTelescopeDatabase:
    ...
    @pytest.mark.asyncio
    async def test_save_auto_discovered_telescope_skipped(self, temp_db, sample_telescope_data):
    ...
    async def test_telescope_exists(self, temp_db, sample_telescope_data):
        ...
        exists = await temp_db.telescope_exists(sample_telescope_data["host"], sample_telescope_data["port"])
    ...
    async def test_delete_telescope(self, temp_db, sample_telescope_data):
        ...
        # Delete telescope
        result = await temp_db.delete_telescope(sample_telescope_data["host"], sample_telescope_data["port"])
        assert result is True
    ...
    async def test_configuration_operations(self, temp_db):
        ...
        # Save configuration
        result = await temp_db.save_configuration(config_name, config_description, config_data)
        assert result is True
    ...
    async def test_database_schema_validation(self, temp_db):
        ...
        async with aiosqlite.connect(temp_db.db_path) as db:
            ...
            expected_columns = [
                "id", "host", "port", "serial_number", "product_model", 
                "ssid", "location", "discovery_method", "created_at", "updated_at"
            ]
    ...
    async def test_concurrent_operations(self, temp_db):
        ...
        results = await asyncio.gather(*[
            save_telescope(i) for i in range(5)
        ])
