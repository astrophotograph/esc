...
@pytest.mark.skipif(not CONTROLLER_ENDPOINTS_AVAILABLE, reason="Controller endpoints not available")
class TestControllerEndpointsComprehensive:
    ...
    def client(self, controller):
        ...
        async def get_telescopes():
            ...
            for telescope in controller.telescopes.values():
                ...
                if hasattr(telescope, 'location'):
                    try:
                        location = await telescope.location if hasattr(telescope.location, '__call__') else telescope.location
                    except:
                        location = "Unknown"
        ...
        async def add_telescope_endpoint(telescope_request: AddTelescopeRequest):
            ...
            if telescope_request.serial_number:
                for existing_telescope in controller.telescopes.values():
                    if getattr(existing_telescope, 'serial_number', None) == telescope_request.serial_number:
            ...
            for existing_telescope in controller.telescopes.values():
                if (existing_telescope.host == telescope_request.host and 
                    existing_telescope.port == telescope_request.port):
                    raise HTTPException(status_code=409,
        ...
        async def remove_telescope_endpoint(telescope_name: str):
            ...
            # Check if telescope exists in remote telescopes
                ...
                return {"status": "success", "message": f"Remote telescope {telescope_name} removed"}
        ...
        async def connect_all_telescopes_endpoint():
            ...
            try:
                ...
                connected_count = sum(1 for t in controller.telescopes.values() 
                                    if hasattr(t, 'client') and t.client and getattr(t.client, 'is_connected', False)
                                    and not (isinstance(t, TestTelescope) or t.port == 9999))
                
                total_count = sum(1 for t in controller.telescopes.values() 
                                if not (isinstance(t, TestTelescope) or t.port == 9999))
                ...
                from fastapi import HTTPException
                raise HTTPException(status_code=500, detail=f"Failed to connect to telescopes: {str(e)}")
        ...
        async def add_remote_controller_endpoint(controller_request: AddRemoteControllerRequest):
            ...
            try:
                ...
                from fastapi import HTTPException
                raise HTTPException(status_code=500, detail=f"Error adding remote controller: {str(e)}")
    ...
    def test_get_telescopes_comprehensive(self, client, controller):
        ...
        local_telescope = next(t for t in telescopes if t["name"] == "comprehensive_scope")
    ...
    def test_add_telescope_comprehensive_validation(self, client, controller):
        ...
        with patch.object(controller, 'add_telescope', new=AsyncMock()) as mock_add_telescope:
