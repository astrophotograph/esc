...
try:
    from main import Controller, AddTelescopeRequest, SaveConfigurationRequest, AddRemoteControllerRequest
...
class TestControllerRunner:
    ...
    async def test_runner_initialization_sequence(self, controller, mock_app):
        ...
        with patch.object(controller, 'load_saved_telescopes', new=AsyncMock()) as mock_load_telescopes:
            with patch.object(controller, 'load_saved_remote_controllers', new=AsyncMock()) as mock_load_controllers:
                with patch.object(controller, 'add_test_telescope', new=AsyncMock()) as mock_add_test:
                    with patch('webrtc_router.initialize_webrtc_service') as mock_webrtc_init:
                        with patch('asyncio.create_task') as mock_create_task:
                            ...
                            async def mock_runner():
                                ...
                                def get_telescope(name): return controller.telescopes.get(name)
    ...
    async def test_runner_with_discovery_enabled(self, mock_app):
        ...
        with patch('main.TelescopeDatabase') as mock_db:
            ...
            with patch.object(controller, 'load_saved_telescopes', new=AsyncMock()):
                with patch.object(controller, 'load_saved_remote_controllers', new=AsyncMock()):
                    with patch.object(controller, 'add_test_telescope', new=AsyncMock()):
                        with patch.object(controller, 'auto_discover', new=AsyncMock()) as mock_auto_discover:
                                ...
                                with patch('asyncio.create_task') as mock_create_task:
                                    ...
                                    async def simplified_runner():
                                        ...
                                        if controller.discover:
                                            asyncio.create_task(controller.auto_discover())
    ...
    async def test_startup_event_handler(self, controller):
        ...
        with patch('websocket_router.websocket_manager') as mock_ws_manager:
            ...
            with patch.object(controller, 'connect_all_telescopes', new=AsyncMock()) as mock_connect_all:
    ...
    async def test_shutdown_event_handler(self, controller):
        ...
        with patch('websocket_router.websocket_manager') as mock_ws_manager:
            ...
            with patch('webrtc_router.cleanup_webrtc_service', new=AsyncMock()) as mock_cleanup:
...
class TestControllerAPIEndpoints:
    ...
    def test_app(self, controller):
        ...
        async def get_telescopes():
            ...
            for telescope in controller.telescopes.values():
                ...
                result.append({
                    ...
                    "host": telescope.host,
                    "port": telescope.port,
                    "location": await telescope.location if hasattr(telescope, 'location') else "Unknown",
                    "connected": getattr(telescope.client, 'is_connected', False) if hasattr(telescope, 'client') else False,
                    "serial_number": telescope.serial_number,
                    "product_model": telescope.product_model,
                    "ssid": getattr(telescope, 'ssid', None),
                    "discovery_method": getattr(telescope, 'discovery_method', 'manual'),
                    "is_remote": False
                })
        ...
        async def add_telescope_endpoint(telescope_request: AddTelescopeRequest):
            ...
            if telescope_request.serial_number and telescope_request.serial_number in controller.telescopes:
            ...
            new_telescope.name = telescope_request.serial_number or telescope_request.host
        ...
        async def remove_telescope_endpoint(telescope_name: str):
            if telescope_name not in controller.telescopes and telescope_name not in controller.remote_telescopes:
                from fastapi import HTTPException
...
class TestControllerHTMLEndpoint:
    ...
    def test_app(self, controller):
        ...
        async def root():
            ...
            local_telescope_count = sum(1 for t in controller.telescopes.values() 
            ...
            auto_discovered_count = sum(1 for t in controller.telescopes.values() 
                                       if getattr(t, 'discovery_method', '') == "auto_discovery" and not (isinstance(t, TestTelescope) or t.port == 9999))
            manual_count = sum(1 for t in controller.telescopes.values() 
                             if getattr(t, 'discovery_method', '') == "manual" and not (isinstance(t, TestTelescope) or t.port == 9999))
...
class TestControllerEndpointErrorHandling:
    ...
    def test_telescope_name_validation(self, controller):
        ...
        request_no_sn = AddTelescopeRequest(
            host="192.168.1.100",
            port=4700
        )
