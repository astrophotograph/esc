...
class TestSeestarClientIntegration:
    ...
    def client(self, mock_connection):
        ...
        with patch('smarttel.seestar.client.SeestarConnection', return_value=mock_connection):
...
class TestSeestarConnection:
    ...
    async def test_message_receiving(self, connection):
        ...
        # Mock connected state
        mock_reader = AsyncMock()
        mock_reader.readuntil.return_value = b'{"id": "resp_123", "result": "success"}\n'
...
class TestTelescopeClientManager:
    ...
    def test_telescope_state_management(self):
        ...
        # Test state queries
        connected_telescopes = [tid for tid, state in telescope_states.items() 
                              if state["connected"]]
        assert len(connected_telescopes) == 2
        
        idle_telescopes = [tid for tid, state in telescope_states.items() 
