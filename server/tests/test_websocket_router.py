...
class TestWebSocketRouterEndpoints:
    ...
    def test_websocket_health_endpoint(self, client, mock_manager):
        ...
        mock_connection.subscriptions = {
            "telescope_1": {SubscriptionType.STATUS, SubscriptionType.IMAGING}
        }
    ...
    async def test_test_broadcast_endpoint(self, client, mock_manager):
        ...
        response = client.post(
            "/api/ws/test/broadcast?telescope_id=test_telescope&message=Hello"
        )
...
class TestWebSocketConnectionHandling:
    ...
    async def test_handle_websocket_connection_success(
        ...
        await _handle_websocket_connection(
            mock_websocket, telescope_id, client_id, mock_manager
        )

        # Verify connection lifecycle
        mock_websocket.accept.assert_called_once()
        mock_manager.connect.assert_called_once_with(
            mock_websocket, client_id, skip_accept=True
        )
    ...
    @pytest.mark.asyncio
    async def test_handle_websocket_connection_no_telescope_id(
        self, mock_websocket, mock_manager
    ):
    ...
    @pytest.mark.asyncio
    async def test_handle_websocket_connection_accept_failure(
        self, mock_websocket, mock_manager
    ):
        ...
        await _handle_websocket_connection(
            mock_websocket, "telescope_1", "client_1", mock_manager
        )
    ...
    @pytest.mark.asyncio
    async def test_handle_websocket_connection_manager_connect_failure(
        self, mock_websocket, mock_manager
    ):
    ...
    @pytest.mark.asyncio
    async def test_handle_websocket_connection_dead_connection(
        self, mock_websocket, mock_manager
    ):
    ...
    @pytest.mark.asyncio
    async def test_handle_websocket_connection_message_handling_error(
        self, mock_websocket, mock_manager
    ):
    ...
    @pytest.mark.asyncio
    async def test_handle_websocket_connection_websocket_disconnect_during_handshake(
        self, mock_websocket, mock_manager
    ):
...
@pytest.mark.skipif(
    not WEBSOCKET_ROUTER_AVAILABLE or not WEBSOCKET_COMPONENTS_AVAILABLE,
    reason="WebSocket components not available",
)
