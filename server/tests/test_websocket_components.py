...
class TestWebSocketManager:
    ...
    async def test_add_connection(self, ws_manager, mock_websocket):
        ...
        try:
            ...
            if hasattr(ws_manager, 'add_connection'):
                await ws_manager.add_connection(mock_websocket, "test_client", "test_telescope")
            elif hasattr(ws_manager, 'register_connection'):
                await ws_manager.register_connection(mock_websocket, "test_client", "test_telescope")
    ...
    def test_manager_properties(self, ws_manager):
        ...
        connection_attrs = ['connections', 'clients', 'active_connections', '_connections']
        has_connection_attr = any(hasattr(ws_manager, attr) for attr in connection_attrs)
        assert has_connection_attr, "Manager should have some connection tracking attribute"


@pytest.mark.skipif(not WEBSOCKET_PROTOCOL_AVAILABLE, reason="WebSocket protocol not available")
...
@pytest.mark.skipif(not PROTOCOL_HANDLERS_AVAILABLE, reason="Protocol handlers not available")
class TestProtocolHandlers:
    ...
    def test_handler_initialization(self, protocol_handler):
        ...
        has_handler = any(hasattr(protocol_handler, method) for method in handler_methods)
...
class TestWebSocketIntegration:
    ...
    async def test_websocket_component_integration(self):
        ...
        assert len(imported_components) > 0, "Should be able to import at least one WebSocket component"
