    ...
    try:
        async with httpx.AsyncClient(timeout=5.0) as client:
            ...
            if response.status_code == 200:
                ...
                print(f"✅ Remote controller reachable, found {len(telescopes)} telescopes:")
                for telescope in telescopes:
                    print(f"   - {telescope.get('name')}: {telescope.get('serial_number')}")
            else:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                for telescope in local_telescopes:
                    ...
                    print(f"   - {name}: {'remote' if is_remote else 'local'}, {'connected' if connected else 'disconnected'}")
                    if name == "870c9918":
                        target_found = True
                        print(f"     🎯 Target telescope found: remote={is_remote}, connected={connected}")
                
                if not target_found:
                    print("❌ Target telescope 870c9918 not found in local server list!")
                    return
            else:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                for controller in controllers:
                    ...
                    print(f"   - {host}:{port}: {status}, {telescopes_count} telescopes")
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                print(f"   Registered telescopes: {health.get('registered_telescopes')}")
            ...
            if parsed.get("type") == "command_response":
                ...
                if success:
                    ...
                    if "not available" in str(error):
                        print("🔍 Analysis: Telescope not registered in WebSocket manager")
                        print("   This means the remote controller registration didn't work properly")
            else:
