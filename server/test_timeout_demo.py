...
async def demo_connection_timeout():
    ...
    print(f"Attempting to connect to {conn.host}:{conn.port} with {conn.connection_timeout}s timeout...")
    ...
    try:
        ...
        print(f"✅ Connection timed out after {elapsed:.2f}s (expected ~{conn.connection_timeout}s)")
...
async def demo_client_timeout():
    ...
    print(f"Fast client timeouts: conn={fast_client.connection_timeout}s, read={fast_client.read_timeout}s, write={fast_client.write_timeout}s")
    print(f"Slow client timeouts: conn={slow_client.connection_timeout}s, read={slow_client.read_timeout}s, write={slow_client.write_timeout}s")
...
async def demo_imaging_client_timeout():
    ...
    print(f"Imaging client timeouts: conn={imaging_client.connection_timeout}s, read={imaging_client.read_timeout}s, write={imaging_client.write_timeout}s")
...
async def demo_timeout_error_handling():
    ...
    for error in other_errors:
        ...
        print(f"{type(error).__name__} classified as connection reset error: {is_connection_error}")
...
def demo_timeout_configuration():
    ...
    print(f"Default timeouts: conn={conn_default.connection_timeout}s, read={conn_default.read_timeout}s, write={conn_default.write_timeout}s")
    ...
    print(f"Custom timeouts: conn={conn_custom.connection_timeout}s, read={conn_custom.read_timeout}s, write={conn_custom.write_timeout}s")
    ...
    print(f"Fast timeouts: conn={conn_fast.connection_timeout}s, read={conn_fast.read_timeout}s, write={conn_fast.write_timeout}s")
