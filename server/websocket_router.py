...
async def websocket_endpoint(
    websocket: WebSocket,
    telescope_id: Optional[str] = Query(
        None, description="Specific telescope to connect to"
    ),
...
async def _handle_websocket_connection(
    websocket: WebSocket, telescope_id: Optional[str], client_id: Optional[str], manager
):
    """
    ...
    if telescope_id:
        logger.info(
            f"WebSocket connection attempt for telescope {telescope_id}, client: {connection_id}"
        )
    else:
        logger.info(
            f"WebSocket connection attempt (no telescope specified), client: {connection_id}"
        )
        ...
        while True:
            try:
                ...
                if not connection.is_alive:
                    logger.warning(
                        f"Received message on dead connection {connection_id}, breaking"
                    )
                    break
                ...
                logger.debug(
                    f"Breaking connection loop for {connection_id} due to error"
                )
                break
...
async def websocket_debug(manager=Depends(get_websocket_manager_dependency)):
    ...
    return {
        ...
        "total_registered": len(manager.telescope_clients)
        + len(manager.remote_clients),
