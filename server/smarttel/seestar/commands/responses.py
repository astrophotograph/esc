    ...
    def parse_result_as(self, model_class: type[BaseModel]) -> Optional[BaseModel]:
        ...
        try:
            if isinstance(self.result, dict):
                ...
                return parsed
            elif isinstance(self.result, (list, tuple)) and hasattr(model_class, '__annotations__'):
                ...
                if len(self.result) <= len(field_names):
                    result_dict = {field_names[i]: self.result[i] for i in range(len(self.result))}
...
class TelescopeMessageParser:
    ...
    @staticmethod
    def _parse_command(data: Dict[str, Any], raw_message: str, timestamp: str) -> ParsedCommand:
    ...
    @staticmethod
    def _parse_response(data: Dict[str, Any], raw_message: str, timestamp: str) -> ParsedResponse:
    ...
    @staticmethod
    def _parse_event(data: Dict[str, Any], raw_message: str, timestamp: str) -> ParsedEvent:
    ...
    @staticmethod
    def create_enhanced_response(response_data: Dict[str, Any]) -> EnhancedCommandResponse:
        ...
        elif method in ["scope_get_equ_coord", "scope_get_ra_dec", "scope_get_horiz_coord"]:
