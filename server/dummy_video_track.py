...
class DummyVideoTrack(VideoStreamTrack):
    ...
    def __init__(self, target_fps: int = 30):
        ...
        logger.info(f"Created dummy video track with target FPS: {target_fps}, track ID: {id(self)}, kind: {self.kind}")
    ...
    def _generate_test_pattern(self) -> np.ndarray:
        ...
        try:
            ...
            cv2.putText(frame, text, (50, 50), font, 1.5, (255, 255, 255), 3, cv2.LINE_AA)
            
            # Timestamp
            timestamp = datetime.now().strftime("%H:%M:%S.%f")[:-3]
            cv2.putText(frame, timestamp, (50, 100), font, 1, (255, 255, 255), 2, cv2.LINE_AA)
            
            # FPS indicator
            fps_text = f"Target FPS: {self.target_fps}"
            cv2.putText(frame, fps_text, (50, 150), font, 1, (255, 255, 255), 2, cv2.LINE_AA)
            
            # Track status
            status_text = f"Started: {self._started}"
            cv2.putText(frame, status_text, (50, 200), font, 1, (255, 255, 255), 2, cv2.LINE_AA)
            
            # Moving bars for motion detection
            bar_pos = int((self.frame_count * 5) % self.width)
            cv2.rectangle(frame, (bar_pos, self.height - 100), (bar_pos + 20, self.height), (255, 0, 0), -1)
            ...
            error_frame = np.full((self.height, self.width, 3), [128, 0, 128], dtype=np.uint8)
            cv2.putText(error_frame, "ERROR", (self.width//2 - 100, self.height//2), 
                       cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 255, 255), 3)
            return error_frame
    
    async def recv(self) -> VideoFrame:
        """Receive the next video frame."""
        logger.debug(f"Dummy track: Received request to receive next video frame #{self.frame_count}")
    ...
    async def stop(self):
        ...
        logger.info(f"Stopped dummy video track {id(self)} after {self.frame_count} frames")
...
class StaticTestVideoTrack(VideoStreamTrack):
    ...
    async def stop(self):
        """Stop the video track."""
        self._started = False
        logger.info(f"Stopped static test video track {id(self)} after {self.frame_count} frames")