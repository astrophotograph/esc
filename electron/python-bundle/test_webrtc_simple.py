...
async def test_webrtc_basic():
    ...
    for i in range(10):
        ...
        logger.info(
            f"Check #{i + 1}: ICE state = {state}, candidates = {candidate_count}"
        )
        if state == "complete":
            break
    ...
    if candidate_count == 0:
        ...
        async def on_candidate2(candidate):
            ...
            if candidate:
                candidate_count2 += 1
                logger.info(
                    f"STUN Candidate #{candidate_count2}: {candidate.candidate}"
                )
            else:
        ...
        for i in range(10):
            ...
            logger.info(
                f"STUN Check #{i + 1}: ICE state = {state}, candidates = {candidate_count2}"
            )
            if state == "complete":
                break
