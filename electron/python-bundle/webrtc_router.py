...
async def create_webrtc_session(
    request: WebRTCCreateSessionRequest,
) -> WebRTCCreateSessionResponse:
    """
    ...
    logger.info(
        f"Creating WebRTC session for telescope: {request.telescope_name}, stream type: {request.stream_type}"
    )
    ...
    try:
        ...
        logger.info(
            f"Successfully created WebRTC session {session_id} for telescope {request.telescope_name}"
        )
        return WebRTCCreateSessionResponse(session_id=session_id, answer=answer)
    except Exception as e:
        logger.error(
            f"Failed to create WebRTC session for telescope {request.telescope_name}: {e}"
        )
        import traceback
...
async def get_webrtc_config() -> dict:
    ...
    return {"iceServers": [{"urls": webrtc_service.stun_servers}]}
