...
export function VintageTestPattern({
  width = 800,
  height = 600,
  className = '',
  statusText,
}: VintageTestPatternProps) {
  ...
  useEffect(() => {
    ...
    const drawResolutionChart = (x: number, y: number, size: number) => {
      ctx.strokeStyle = '#FFFFFF'
      ctx.lineWidth = 1
    ...
    const drawFrequencyBars = (
      ...
      for (let i = 0; i < barCount; i++) {
        ...
        if (vertical) {
          ctx.fillRect(x, y + (i * barHeight) / barCount, barWidth, barHeight / barCount)
        } else {
          ctx.fillRect(x + (i * barWidth) / barCount, y, barWidth / barCount, barHeight)
        }
      }
    }
    ...
    drawResolutionChart(cornerOffset, cornerOffset, cornerSize)
    // Top-right corner pattern
    drawResolutionChart(width - cornerOffset - cornerSize, cornerOffset, cornerSize)
    // Bottom-left corner pattern
    drawResolutionChart(cornerOffset, height - cornerOffset - cornerSize, cornerSize)
    // Bottom-right corner pattern
    drawResolutionChart(
      width - cornerOffset - cornerSize,
      height - cornerOffset - cornerSize,
      cornerSize
    )

    // Draw frequency bars on the sides (like old test patterns)
    drawFrequencyBars(20, height * 0.25, 30, height * 0.5, true) // Left side
    drawFrequencyBars(width - 50, height * 0.25, 30, height * 0.5, true) // Right side

    // Draw geometric patterns in corners
    drawGeometricPattern(centerX - maxRadius * 0.7, centerY - maxRadius * 0.7, 40)
    drawGeometricPattern(centerX + maxRadius * 0.7 - 40, centerY - maxRadius * 0.7, 40)
    drawGeometricPattern(centerX - maxRadius * 0.7, centerY + maxRadius * 0.7 - 40, 40)
    drawGeometricPattern(centerX + maxRadius * 0.7 - 40, centerY + maxRadius * 0.7 - 40, 40)
    ...
    if (statusText) {
      ctx.fillText(statusText, centerX, height * 0.87)
    } else {
    ...
    ctx.textAlign = 'left'
    ctx.fillText(`RESOLUTION: ${width}x${height}`, 20, height - 60)
