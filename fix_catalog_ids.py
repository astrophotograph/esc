        ...
        if obj_id.startswith('NGC'):
            ...
            if ngc_match:
                ...
                for m_id, ngc_id in MESSIER_NGC_MAPPING.items():
                    # Only check NGC cross-references here
                    if ngc_id.startswith('NGC'):
                        # Handle both with and without leading zeros
                        ngc_base = ngc_id.replace('NGC', '')
                        if (ngc_id == f"NGC{ngc_num}" or 
                            ngc_id == f"NGC{int(ngc_num)}" or
                            f"NGC{int(ngc_base)}" == f"NGC{int(ngc_num)}"):
        ...
        # Fix IC objects
            ...
            if ic_match:
                ...
                for m_id, cat_id in MESSIER_NGC_MAPPING.items():
                    # Handle both with and without leading zeros
                    if cat_id.startswith('IC'):
                        ic_base = cat_id.replace('IC', '')
                        if (cat_id == f"IC{ic_num}" or 
                            cat_id == f"IC{int(ic_num)}" or
                            f"IC{int(ic_base)}" == f"IC{int(ic_num)}"):
                            if not catalog_ids.get('messier'):
                                catalog_ids['messier'] = m_id
                                stats['messier_cross_refs'] += 1
                                stats['total_fixed'] += 1
...
if __name__ == "__main__":
    ...
    if not Path(catalog_path).exists():
        ...
        print("Please run this script in the directory containing the catalog file")
        exit(1)
